...
from src.components.pages.actions.user_profile_actions import fetch_user_profile, upload_profile_picture, change_profile, save_profile_changes


class Edit_Profile_Frame(Main_Container):
    def __init__(self, master, user_id, *args, **kwargs):
        ...
        self.edit_user_profile_container()
    ...
    def edit_user_profile_container(self):
        try:
            ...
            ctk.CTkLabel(header, text="Edit User Profile", font=("Poppins", 24, "bold"), text_color="#2F2F2F").pack(side="left", padx=(20, 0), pady=(30, 10))
            ...
            self.first_name_ent = ctk.CTkEntry(name_frame, placeholder_text=first_name, font=("Poppins", 24), text_color="#2F2F2F", fg_color="#FFFFFF",  width=200)
            self.first_name_ent.pack(side="left", padx=(80, 0), pady=(10, 10))

            self.last_name_ent = ctk.CTkEntry(name_frame, placeholder_text=last_name, font=("Poppins", 24), 
                                              text_color="#2F2F2F", fg_color="#FFFFFF", width=200)
            self.last_name_ent.pack(side="left", padx=(10, 0), pady=(10, 10))
            ...
            self.username_ent= ctk.CTkEntry(username_fr, placeholder_text=username, font=("Poppins", 24), text_color="#2F2F2F", fg_color="#FFFFFF", width=411)
            self.username_ent.pack(side="left", padx=(25, 0), pady=(10, 10))
            ...
            self.address_ent = ctk.CTkEntry(address_fr, placeholder_text="e.g. 123 Mabini St., Quezon City", font=("Poppins", 24), text_color="#2F2F2F", fg_color="#FFFFFF", width=411)
            self.address_ent.pack(side="left", padx=(50, 0), pady=(10, 10))
            ...
            self.email_ent = ctk.CTkEntry(email_fr, placeholder_text=email, font=("Poppins", 24), text_color="#2F2F2F", fg_color="#FFFFFF", width=411)
            self.email_ent.pack(side="left", padx=(80, 0), pady=(10, 10))
            ...
            self.phone_ent =ctk.CTkEntry(phone_fr, placeholder_text="e.g. 0917 123 4567", font=("Poppins", 24), text_color="#2F2F2F", fg_color="#FFFFFF", width=411)
            self.phone_ent.pack(side="left", padx=(10, 0), pady=(10, 10))
            ...
            def save_profile():
                  result = save_profile_changes(
                        self.username_ent.get(),  # username
                        self.first_name_ent.get(),  # first_name
                        self.last_name_ent.get(),  # last_name
                        self.email_ent.get(),  # email
                        self.phone_ent.get(),  # phone
                        self.address_ent.get(),  # address
                        self.user_id  # user_id
                  )
            ...
            save_btn = ctk.CTkButton(btn_fr, text="Save Changes", font=("Poppins", 21, "bold"),
                                     text_color="#FFFFFF", fg_color="#507DBC", hover_color="#789ADA",
                                     width=160, height=40, cursor="hand2",
                                     command=save_profile)
            save_btn.pack(side="left", padx=(10, 30), pady=(10, 10))
            ...
            print(f"Error @ edit_user_profile.py: {e}")
